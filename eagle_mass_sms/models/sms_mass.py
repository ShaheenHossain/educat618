...
class SmsMass(models.Model):
    ...
    sms_template_id = fields.Many2one('sms.template', string="Template",required="False")
    ...
    def send_mass_sms(self):
        ...
        for rec in self.selected_records:
            modelName=self.sms_template_id.model
            modelRecordId=self.env[modelName].search([['partner_id','=',rec.id]])
            # self.env('modelName').search([("partner_id.id", "=", rec.id)]).id
            sms_rendered_content = self.env['sms.template'].render_template(self.message_text, modelName, modelRecordId.id)
            ...
            sms_queue = self.env['ir.model.data'].get_object('eagle_sms', 'sms_queue_check')
